<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800">Tutores Eliminados</h2>
    </x-slot>
                ...
                <table class="min-w-full divide-y divide-gray-200">
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        @forelse($tutores as $tutor)
                            <tr>
                                <td class="px-6 py-4 whitespace-nowrap">{{ $tutor->nombre }}
                                    {{ $tutor->paterno }} {{ $tutor->materno }}</td>
                                <td class="px-6 py-4 whitespace-nowrap">{{ $tutor->email }}</td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <form action="{{ route('tutores.restore', $tutor->id) }}" method="POST"
                                        onsubmit="return confirm('¿Estás seguro de restaurar este tutor?')">
                                        @csrf
                        ...
                        @empty
                            <tr>
                                <td colspan="3" class="px-6 py-4 text-center">No hay tutores eliminados.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
